...
const contractBytes = Buffer.from(encode(contract.value));
...
const devcontainer = `${JSON.stringify(
  {
    ...
    postCreateCommand: "prismpm template check && prismpm lock check && just setup && just vv",
  },
  null,
  2,
)}\n`;
...
for (const name of workflowNames) {
  ...
  renderedWorkflow = renderedWorkflow.replace(
    /UOR-Foundation\/PrismPM\/\.github\/workflows\/sdk\.yml@(?:__PRISMPM_ACTION_COMMIT__|[0-9a-f]{40})/g,
    `UOR-Foundation/PrismPM/.github/workflows/sdk.yml@${actionReference.split('@')[1]}`,
  );
  if (previousSdkImage) {
...
const rendered = new Map([
  ...
  ["template-contract.json", contractBytes],
]);
const policyFiles = [];
...
const templateLock = encode({
  contract_digest: sha(contractBytes),
...
await Promise.all([
  ...
  writeFile("/workspace/standards.lock", standards),
  writeFile("/workspace/template-contract.json", contractBytes),
