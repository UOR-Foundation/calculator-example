...
Finalizing this Calculator instantiation passes four explicit arguments to
`bootstrap/render.sh`: the SDK manifest-list digest, the shared action's
independently published commit, the reviewed template policy-input commit, and
the public PrismPM runtime manifest-list digest. No
...
devcontainer, workflow pins, authoritative A/B runtime bindings, and dependent
LexLean lock. Calculator's source inputs must be committed and clean before the
transaction; a lock-generation failure restores both the system model and
LexLean lock. The template revision names the external template policy input,
not the containing Calculator revision.
